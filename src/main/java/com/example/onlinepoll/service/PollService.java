...
public class PollService {
    ...
    public Poll findByLink(String link) {
        Poll poll = pollRepository.findByLink(link)
                .orElseThrow(() -> new InstanceNotFoundException("Not poll founded with link: " + link));
        Set<Participant> sortedParticipant = poll.getParticipants().stream()
                .sorted(Comparator.comparing(Participant::getCreatedAt))
                .collect(Collectors.toCollection(LinkedHashSet::new));
        Set<Option> sortedOptions = poll.getOptions().stream().sorted(Comparator.comparing(Option::getId))
                .collect(Collectors.toCollection(LinkedHashSet::new));
